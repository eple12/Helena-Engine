...
static void trace_pawn_value(const Board& board, Trace& trace, Color color) {
    ...
    while (friendly_pawns_copy) {
        ...
        if (!(EvalConstants::AdjacentFiles[sq.file()] & friendly_pawns)) {
            num_isolated++;
        }
    }
...
parameters_t HelenaEval::get_initial_parameters() {
    ...
    get_initial_parameter_single(parameters, EvalConstants::IsolatedPawnPenaltyPerPawn);
...
coefficients_t get_coefficients(const Trace& trace) {
    ...
    coeffs.push_back(-(trace.isolated_pawn_penalty[0] - trace.isolated_pawn_penalty[1]));
...
void HelenaEval::print_parameters(const parameters_t& parameters) {
    ...
    // Mop-up parameters are not printed as they are not tuned
    ...
    print_single(ss, parameters, index, "IsolatedPawnPenaltyPerPawn");
    ss << endl;
    ss << "    // King Safety" << endl;
